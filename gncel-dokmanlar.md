...
- **Yetkilendirme Seviyeleri:** Klasörlerde, en üst seviyeden en alt dokümana kadar farklı seviyelerde yetkilendirme yapılabilir.
- **Klasör Erişimi:** Kullanıcılara klasör yetkisi verilerek, klasör ve içindeki dokümanlara erişimleri sağlanabilir.
- **Doküman Bazlı Yetkilendirme:** Klasör içerisindeki dokümanlar için ayrı ayrı yetkilendirme ayarlanabilir.
- **Detaylı Bilgi:**
  - Klasör yetkilendirmeleri için: **Parametreler → Klasör Tanımlama**
  - Doküman yetkilendirmeleri için: **Klasör Detay Sayfası → Doküman
    Künyesi**
...
- Dokümanlar, kullanıcı ekranında görsel olarak listelenir.
- Alanların aktif veya pasif olması, kullanıcının **yetkilerine
  bağlıdır**.
- Kullanıcının doküman bazında sahip olduğu **indirme, yazdırma ve diğer
  yetkiler**, ekran üzerinde ilgili alanların görünürlüğünü belirler.
...
- Kullanıcılar, dokümanın hangi klasör içinde bulunduğunu bu alan üzerinden kolayca görebilir.
...
- Kullanıcılar, dokümanın hangi formatta olduğunu bu alan üzerinden hızlıca görebilir.
...
- Favori dokümanlara, **menüde yer alan "Favori Dokümanlarım"** alanından erişilebilir.
- Ayrıca favori dokümanlara, **mobil uygulama üzerinden çevrimdışı
  (offline)** olarak da ulaşılabilir.
...
- **Açık:** Doküman üzerinde işlem yapılabilir.
- **Kapalı:** Doküman üzerinde şu anda devam eden bir işlem vardır ve bu süre boyunca doküman üzerinde değişiklik yapılamaz.
...
1. Kullanıcı **Arşivle** butonuna tıkladığında bir bilgilendirme ekranı açılır.
2. Kullanıcı, açılan ekranda **"Evet, Arşivle"** seçimini yapar.
3. Sistem, dokümanı **Doküman Arşivi** alanında, mevcut klasörünün ağaç yapısına uygun bir alt klasöre taşır.
4. Dokümanın **statüsü otomatik olarak "Arşivde"** olarak güncellenir.
5. Sistem, dokümanın arşive alınmasına ilişkin açıklamayı, arşive alan kişiyi ve arşive alma tarihini kaydeder ve bu bilgiler doküman künyesinde görüntülenir.
...
- Filigran, dokümanın yetkisiz kullanımını önlemeye yardımcı olur.
- Kullanıcı, filigranlı veya filigransız görüntüleme seçimini yapabilir; sistem seçime göre dokümanı ilgili formatta ekranda gösterir.
...
- Filigran, dokümanın yetkisiz kullanımını önlemeye yardımcı olur.
- Kullanıcı, filigranlı veya filigransız indirme seçimini yapabilir; sistem seçime göre dokümanı ilgili formatta indirir.

**Filigranlı indirme örneği;**
...
- Kullanıcı, ilgili butona tıkladığında yetkisine göre bir bilgilendirme ekranı açılır.
- Dokümanın yazdırma yetkisi, kullanıcının sahip olduğu yetki setine göre belirlenir.
- Yetki tanımlamaları hakkında detaylı bilgi için **Yetki Setleri** sayfasına bakabilirsiniz.

Bu özellik, dokümanların güvenli bir şekilde yazdırılmasını ve yalnızca yetkili kullanıcıların çıktıya erişmesini sağlar.

<img src="https://cdn.document360.io/e072f215-2317-4c24-b7a8-132a798e9cac/Images/Documentation/gncel-dokmanlar-image-s0jpohx2.png" /> Kullanıcının seçimine bağlı olarak doküman, **sistem kurulumunda
tanımlanmış filigran parametrelerine** uygun şekilde yazdırılır.

- Filigran, dokümanın yetkisiz çoğaltılmasını önler.
- Kullanıcı, filigranlı veya filigransız yazdırma seçimini yapabilir; sistem seçime göre dokümanı ilgili parametreye uygun biçimde yazdırır.
...
- Kullanıcı, ilgili butona tıkladığında yetkisine göre bir bilgilendirme ekranı açılır.
- Dokümanın paylaşım yetkisi, kullanıcının sahip olduğu yetki setine göre belirlenir.
- Yetki tanımlamaları hakkında detaylı bilgi için **Yetki Setleri** sayfasına bakabilirsiniz.
...
- Filigran, dokümanın yetkisiz kullanımını ve çoğaltılmasını önler.
- Kullanıcı, filigranlı veya filigransız paylaşım seçimini yapabilir; sistem seçime göre dokümanı ilgili parametrelere uygun biçimde paylaşır.
